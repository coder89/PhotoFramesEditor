...
void RotationHandler::mouseMoveEvent(QGraphicsSceneMouseEvent * event)
{
    ...
    qDebug() << scale*currentPos-lastPos;
    m_rotation_angle;
    if (currentPos.ry()>0)
...
RotationWidget::RotationWidget(QGraphicsItem * parent):
    QGraphicsWidget(parent),
...
void RotationWidget::paint(QPainter * painter, const QStyleOptionGraphicsItem * option, QWidget * widget)
{
...
QPainterPath RotationWidget::shape() const
{
    return m_path + m_handler->shape().translated(m_handler->pos());
}

QPainterPath RotationWidget::opaqueArea() const
{
    return m_path + m_handler->shape().translated(m_handler->pos());
}
QRectF RotationWidget::boundingRect() const
{
    return (m_path + m_handler->shape().translated(m_handler->pos())).boundingRect();
}

void RotationWidget::hoverEnterEvent(QGraphicsSceneHoverEvent * event)
{
    this->setCursor(QCursor(Qt::OpenHandCursor));
}

void RotationWidget::hoverLeaveEvent(QGraphicsSceneHoverEvent * event)
{
    this->unsetCursor();
}

void RotationWidget::mousePressEvent(QGraphicsSceneMouseEvent * event)
{
    this->setCursor(QCursor(Qt::ClosedHandCursor));
    QGraphicsWidget::mousePressEvent(event);
}

void RotationWidget::mouseReleaseEvent(QGraphicsSceneMouseEvent * event)
{
    this->setCursor(QCursor(Qt::OpenHandCursor));
    QGraphicsWidget::mouseReleaseEvent(event);
}

void RotationWidget::mouseMoveEvent(QGraphicsSceneMouseEvent * event)
{
...
void RotationWidget::emitRotationChanges(qreal deg, bool round)
{
    this->update();
