...
void LayersTree::setRowWidgets(const QModelIndex & parent, int row)
{
    ...
    SwitchableIcon * l1 = new SwitchableIcon(data1.value<QIcon>());
    ...
    SwitchableIcon * l2 = new SwitchableIcon(data2.value<QIcon>());
    l2->toggle();
    connect(l2, SIGNAL(clicked(SwitchableIcon*)), this, SLOT(hide(SwitchableIcon*)));
    this->setIndexWidget(model()->index(row, 2, parent), l2);
}

void LayersTree::hide(SwitchableIcon * item)
{
    //item->setVisible(!item->isVisible());
}

void LayersTree::lock(SwitchableIcon * item)
{
    //item->setFlag(QGraphicsItem::ItemIsMovable, item->flags() ^ QGraphicsItem::ItemIsMovable);
}
